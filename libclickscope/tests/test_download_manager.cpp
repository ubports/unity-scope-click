...
class FakeDownloadManager : public click::DownloadManager
{
    Q_OBJECT
public:
    FakeDownloadManager()
    ...
    MOCK_METHOD4(getAllDownloadsWithMetadata, void(const QString &key, const QString &value,
...
class FakeDownloader : public click::Downloader
{
    ...
    FakeDownloadManager& dm;
public:
    FakeDownloader(const QSharedPointer<click::network::AccessManager>& nam, FakeDownloadManager& dm)
        : Downloader(nam), dm(dm)
    {
...
TEST(ClickDownloaderTest, testDownloadProgress)
{
    FakeDownloadManager fake_dm;
    qDebug() << "so far";
    QSharedPointer<MockNetworkAccessManager> mockNam;
    qDebug() << "this far";

    FakeDownloader downloader(mockNam, fake_dm);
    qDebug() << "that far";

    EXPECT_CALL(fake_dm, getAllDownloadsWithMetadata(_, _, _, _));

    downloader.get_download_progress("fake_package_name", [](std::string object_path) {
        qDebug() << "fake package" << QString::fromStdString(object_path);
    });
}
